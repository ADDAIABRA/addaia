...
def gerar_projetos_ia(perfil, avaliacao):
    ...
    for resp_pilar in avaliacao.respostas_pilares.all():
        ...
        for rp in resp_pilar.respostas_perguntas.select_related("pergunta").all():
            ...
            pilar_info["respostas_detalhadas"].append({
                "pergunta": rp.pergunta.enunciado,
                "resposta_escolhida": getattr(rp.pergunta, alt_attr, "N/A")
            })
        avaliacao_data["pilares"].append(pilar_info)

    # Coletar informações do Relatório gerado pela IA anteriormente
    relatorio_data = {}
    try:
        from relatorio.models import Relatorio
        relatorio = Relatorio.objects.filter(avaliacao=avaliacao).first()
        if relatorio:
            relatorio_data = relatorio.conteudo_json
    except Exception as e:
        logger.warning(f"Não foi possível carregar o relatório para o contexto de projetos: {e}")

    contexto_negocio = f"""
    DADOS DA EMPRESA:
    {json.dumps(perfil_data, indent=2, ensure_ascii=False)}

    DIAGNÓSTICO COMPLETO (QUESTÕES E RESPOSTAS):
    {json.dumps(avaliacao_data, indent=2, ensure_ascii=False)}

    RELATÓRIO ESTRATÉGICO GERADO PELA IA (CONTEXTO ADICIONAL):
    {json.dumps(relatorio_data, indent=2, ensure_ascii=False)}
    """
    ...
    try:
        ...
        for p_data in projetos_data:
            Projeto.objects.create(
                ...
                prazo_minimo=p_data.get("prazo_minimo", ""),
                prazo_maximo=p_data.get("prazo_maximo", ""),
                prazo_estimado=p_data.get("prazo_estimado", ""),
                preco_minimo=p_data.get("preco_minimo", ""),
                preco_maximo=p_data.get("preco_maximo", ""),
                preco_estimado=p_data.get("preco_estimado", "")
            )
