(function ($) {
  ...
  $(window).on("scroll load", animateTargetCards);
...
// Inicialização segura que aguarda o carregamento completo do DOM e Scripts (jQuery/Bootstrap)
    window.addEventListener('load', function() {
        if (typeof jQuery === 'undefined') {
            console.error('Carrossel: jQuery não encontrado.');
            return;
        }

        (function($) {
            var $carousel = $('#nxPlatformCarousel');

            // Forçamos a inicialização via JS para evitar conflitos com data-attributes
            $carousel.carousel({
                interval: 5000,
                ride: 'carousel'
            });
            ...
            $carousel.on('slide.bs.carousel', function (e) {
                $('#nx-current-slide').text(e.to + 1);
            });

            // Clique para zoom (Lightbox)
            $carousel.find('.carousel-item img').on('click', function() {
                var imgSrc = $(this).attr('src');
                $('#nxModalImage').attr('src', imgSrc);
                $('#nxImageModal').modal('show');
            });

            // Suporte para Swipe (Celular)
            var startX = 0;
            ...
            $carousel.on('touchend', function(e) {
                ...
                if (startX - endX < -50) $carousel.carousel('prev');
            });
        })(jQuery);
    });